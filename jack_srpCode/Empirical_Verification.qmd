...
results_list <- lapply(seq_len(n_reps), function(rep) {
  ...
  # Precompute mu_t = Sigma^{-1/2} mu for the integrands
  sigma_inv_half <- solve(sqrtm(cov_matrix))
  mu_t <- as.vector(sigma_inv_half %*% mu)

  # Compute expected error from integral (corrected SURE)
  expectation_1 <- quadinf(joint_mgf, 0, Inf, mu_t = mu_t)$Q
  expectation_2 <- quadinf(integrand_mu_2, 0, Inf, mu_t = mu_t, cov_matrix = cov_matrix)$Q
