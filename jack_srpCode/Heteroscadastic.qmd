...
joint_mgf <- function(t, mu) {
  D <- diag(2*t + 1,nrow = length(mu))
  # Compute terms
...
expectation_1 <- quadinf(joint_mgf, 0, Inf, mu = mu)$Q

integrand_mu_2_1 <- function(t, mu, sigma_vec) {
  inv_D <- solve(diag(2*t + 1,nrow = length(mu)))
  L <- sqrt(inv_D)
  R <- t(L) %*% diag(sigma_vec) %*% L
  
  mu_tilda <- t(L) %*% mu
  second_term <- sum(diag(R)) + t(mu_tilda) %*% R %*% mu_tilda
  
  phi_value <- joint_mgf(t,mu)
...
Mu_scaled_tibble <- Mu_scaled_tibble |> 
  ...
  mutate(
    ...
    Expectation_1 = quadinf(joint_mgf, 0, Inf, mu = unlist(Mu))$Q,
