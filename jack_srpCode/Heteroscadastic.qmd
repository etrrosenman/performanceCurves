...
d <- 3
mu <- c(0,0,0,0,0,0)     # Mean vector
sigma_vec <- c(1,2,3,1,2,3)  # Heteroscedastic variances
...
target_maha_sq <- seq(0.01, 10, by = 0.05)
...
# Baseline risk (MLE, no shrinkage) = trace(Sigma), scaled by dimension
baseline_df <- Mu_scaled_tibble |>
  distinct(Dimension, Trace) |>
  mutate(Baseline = Trace / Dimension)

Mu_scaled_tibble |>
  mutate(
    Scaled_Risk = Expected_Risk / Dimension
  ) |>
  ggplot(aes(x = Squared_L2, y = Scaled_Risk)) +
    geom_hline(data = baseline_df, aes(yintercept = Baseline),
               linetype = "dashed", color = "gray50", linewidth = 0.7) +
    geom_line(aes(color = Direction), linewidth = 1.2) +
    facet_wrap(~ Dimension, scales = "free_y", ncol = 3) +
    scale_color_manual(values = c("Leading" = "#2196F3", "Lagging" = "#E91E63")) +
    labs(
      title = expression("Scaled Expected Risk vs. ||" * Sigma^{-1/2} * mu * "||"^2),
      x = expression("||" * Sigma^{-1/2} * mu * "||"^2),
